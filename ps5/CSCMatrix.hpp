...
public:
  ...
  void worker_CSC(const Vector& x, size_t begin, size_t end, Vector& y) {  
    // Vector partial_y = y;
  ...
  for (size_t i = 0; i < parts; ++i) {
    futs.push_back(std::async(std::launch::async,
                        &CSCMatrix::worker_CSC,*this,std::cref(x),i*blocksize,(i+1)*blocksize, std::ref(y)));                
  ...
  void worker_tCSC(const Vector& x, size_t begin, size_t end, Vector& y) {  
    // Vector partial_y = y;
    for (size_t i = begin; i < end; ++i) {
      double t = y(i);
      for (size_t j = col_indices_[i]; j < col_indices_[i + 1]; ++j) {
  ...
  for (size_t i = 0; i < parts; ++i) {
    futs.push_back(std::async(std::launch::async,
                        &CSCMatrix::worker_tCSC,*this,std::cref(x),i*blocksize,(i+1)*blocksize, std::ref(y)));                
